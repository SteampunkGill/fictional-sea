    ...
    public ResponseEntity<DocumentsResponse> getDocuments(
        ...
        try {
            ...
            for (Map<String, Object> doc : documents) {
                ...
                // 处理 reading_progress 可能为 Double 的情况
                Object progressObj = doc.get("reading_progress");
                if (progressObj instanceof Double) {
                    dto.setReadProgress(((Double) progressObj).intValue());
                } else if (progressObj instanceof Integer) {
                    dto.setReadProgress((Integer) progressObj);
                } else {
                    dto.setReadProgress(0);
                }
                ...
                dto.setCreatedAt(formatDate((Timestamp) doc.get("created_at")));
                dto.setUpdatedAt(formatDate((Timestamp) doc.get("updated_at")));
